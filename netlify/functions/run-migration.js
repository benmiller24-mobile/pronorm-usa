...
exports.handler = async function(event) {
  ...
  // DB password passed at runtime, never stored in code
  ...
  try {
    ...
    const pol = (name, table, op, check, using) => {
      ...
      const sql = op === 'INSERT'
        ? `CREATE POLICY "${name}" ON ${table} FOR INSERT WITH CHECK (${check})`
        : `CREATE POLICY "${name}" ON ${table} FOR ${op} USING (${using})`;
      return client.query(sql);
    };

    const dealerCheck = (tbl, fk = 'dealer_id') =>
      `EXISTS (SELECT 1 FROM dealers d WHERE d.user_id = auth.uid() AND (d.role = 'admin' OR d.id = ${tbl}.${fk} OR d.parent_dealer_id = ${tbl}.${fk}))`;
    const joinCheck = (joinTbl, joinCol, fk = 'dealer_id') =>
      `EXISTS (SELECT 1 FROM ${joinTbl} j JOIN dealers d ON d.user_id = auth.uid() WHERE j.id = %TABLE%.${joinCol} AND (d.role = 'admin' OR d.id = j.${fk} OR d.parent_dealer_id = j.${fk}))`;
