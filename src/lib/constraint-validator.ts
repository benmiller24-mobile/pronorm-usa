...
export function validateLayout(items: MappedItem[], intakeData: IntakeData): ValidationIssue[] {
  ...
  function getCabinetRow(sku: string): 'base' | 'wall' | 'tall' {
    ...
    if (s.startsWith('HP') || s.startsWith('HSP') || s.startsWith('HS') || s.startsWith('HG') || s.startsWith('HGP') || s.startsWith('HR') || s.startsWith('H') || s.startsWith('AH')) return 'tall';
  ...
  for (const wallDef of intakeData.walls) {
    ...
    const VALID_BASE_WIDTHS = [15, 20, 27, 30, 38, 40, 45, 50, 55, 60, 75, 80, 90, 91, 100, 105, 110, 115, 120, 125];
